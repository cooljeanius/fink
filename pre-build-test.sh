...
if [ ! -e /usr/bin/pod2man ] ; then
	...
	# test whether it is a perl script:
	pmtest=`file -L /usr/bin/pod2man | grep perl`
	# if so, and it is executable, give option to make it so:
	if [ "x$pmtest" != "x" ] ; then
		...
		if [ "$REPLY" == "y" ] ; then
			...
			if [ -z "`chmod a+x /usr/bin/pod2man 2>&1`" ] ; then
				...
				printf "FAILED\n"
				printf "\nI was unable to change the permissions of \n"
				printf "/usr/bin/pod2man for some reason.  You will need to \n"
				printf "change them yourself.  Open Disk Utility and run\n"
				printf "'Repair Disk Permissions' on your system hard drive.\n"
				printf "If that fails, then run\n\n"
		...
		#/usr/bin/pod2man is NOT a perl script (probably):
